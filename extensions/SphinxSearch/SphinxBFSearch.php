...
/**
 * Class file for the SphinxBFSearch extension
 *
...
class SphinxBFSearch extends SpecialPage {
	...
	var $page = array();           // results page we are on
	var $searchLang = '';          // which index language to use (from GET)
	var $series = array();         // List of valid series
	...
	function __construct() {
		...
		if ( $wgDisableInternalSearch ) {
			...
			parent::__construct( 'SphinxBFSearch' );
		}

		# Fetch series info from MediaWiki:SeriesGroups page
		$title = Title::newFromText( 'SeriesGroups', NS_MEDIAWIKI );
		$rev = Revision::newFromTitle ( $title );

		if ( $rev === null ) {
			throw new MWException( "The $title page was not found!" );
		}
		...
		foreach ( $lines as $line ) {
			...
			if ( preg_match( '/<!--.*-->/', $line) or (trim($line) === "") ) {
				continue;
			}

			# Match any number of values in the form: * a[,[b]]
			$regex = preg_match( '/^\* ((.*)(, (.*))+)$/', trim($line), $matches);
			
			# Skip processing if this is not a valid syntax
			if ( !$regex ) {
				continue;
			}

			# $matches[1] should hold the accepted result, minus the asterisk
			$sr = explode(',', $matches[1]);

			# trim off any pre/post whitespace
			foreach ( $sr as &$s ) {
				$s = trim($s);
			}

			# Store in $this->series[]
			$this->series[strval($sr[0])] = array(
				"series"  => $sr[0],
				"label"   => $sr[1],
				"checked" => $sr[2],
				"crc32"   => sprintf("%u", crc32($sr[0]))
			);
		}
	...
	/**
	 * Return HTML for select dropdown of search language
	 *
	 * @return string
	 */
	function languageDropdown() {
		global $wgRequest, $wgUser, $wgTitle, $wgParser;
		...
		# Fetch series info from MediaWiki:SeriesGroups page
		$rev = Revision::newFromTitle( Title::newFromText( 'SearchLanguages', NS_MEDIAWIKI ) );
		if ( $rev === null ) {
			return; # Return nothing, no dropdown will be shown
		}
		$lines = explode("\n", $rev->getText() );

		$dropdown  = '<div id="spx_language_search">' . "\n";
		$dropdown .= 'Search Language:<br />' . "\n";
		$dropdown .= '<select name="searchLang">' . "\n";
		foreach ( $lines as $line ) {
			# Skip comments and whitespace lines
			if ( preg_match( '/<!--.*-->/', $line) or (trim($line) === "") ) {
				continue;
			...
			# Match only list values
			$regex = preg_match( '#^\*.*Languages/LangName\|(.*?)\}\}#', trim($line), $matches);
			...
			# $matches[1] should hold the language code
			$langCode = $matches[1];

			# Fetch the text of the language
			$lang = $psr->parse( "{{Template:Languages/LangName|$langCode}}", $wgTitle, $opt, false );

			# If this language is the searched language...
			if ( $langCode === $wgRequest->getVal('searchLang') ) {
				# ...then mark it as "selected"...
				$dropdown .= '<option selected value="' .	$langCode . '">' . $lang->getText() .	"</option>\n";
			} else {
				# ...otherwise, leave it as is
				$dropdown .= '<option value="' . $langCode . '">' . $lang->getText() . "</option>\n";
			}
		}
		$dropdown .= '</select>' . "\n";
		$dropdown .= '</div>' . "\n";
	...
	function execute( $par ) {
		global $wgRequest, $wgOut, $wgUser, $wgSphinxMatchAll, $wgSphinxSearch_index_list, $wgSphinxSearch_index_regex;
		...
		foreach ( $this->series as &$sr ) {
			$sr["checked"] = $wgRequest->getInt( "series{$sr["series"]}", 0 );
		}

		$searchSer = $wgRequest->getVal("searchSer");
		$validSeries = array();
		foreach ( $this->series as &$sr ) {
			$validSeries[] = $sr["series"];
		}
		$validSeries = join("|", $validSeries);
		$validSeries = preg_match("/^($validSeries)$/", $searchSer);
		if ( $searchSer == 'All' ) {
			foreach ( $this->series as &$sr ) {
				$sr["checked"] = 1;
			}
			...
			foreach ( $this->series as &$sr ) {
				if ( $sr["series"] == $searchSer ) {
					$sr["checked"] = 1;
				}
			}
		}

		# Default to "All" options checked if nothing checked
		$nothingChecked = true;
		foreach ( $this->series as &$sr ) {
			if ( $sr["checked"] ) {
		...
		unset($nothingChecked);

		$this->categories = $wgRequest->getIntArray( "cat", array() );
		$this->exc_categories = $wgRequest->getIntArray( "exc", array() );

		# Try fetch new page[] array from GET, otherwise default to page 1
		foreach ( $this->series as &$sr ) {
			if ( $sr["checked"] === 1 ) {
				$this->page[$sr["series"]] = $wgRequest->getInt( "page{$sr["series"]}", 1 );
			}
		}
		...
		if ( preg_match($wgSphinxSearch_index_regex, $wgRequest->getVal('searchLang')) ) {
			# if it passed the regex of allowed languages, then set it
			$this->searchLang = $wgRequest->getVal('searchLang');
		} else {
			# Regex failed for some reason, default it to the main "en" index
			$this->searchLang = 'en';
		}

		$temp_index_list = explode(",", $wgSphinxSearch_index_list);
		foreach ($temp_index_list as &$index) {
			$index .= $this->searchLang;
		}
		$new_index_list = implode(",", $temp_index_list);

		# Add a <ul> for the series tabs
		$wgOut->addHTML( '<ul id="spx_tabs">' );
		foreach ( $this->page as $p => $key ) {
			# Set the <li> to the label for this series
			$li = $this->series[strval($p)]["label"];
			$p_parsed = preg_replace('/\./', '_', $p);
			$wgOut->addHTML( '<li><a href="#sphinx-group-' . $p_parsed . '">' . $li . '</a></li>' );
		}
		$wgOut->addHTML( '</ul>' );

		# Wrap all search results in a <div>
		$wgOut->addHTML( '<div id="spx_tabs_container">' );
		# Only loop through pages which. Set based on series selection
		foreach ( $this->page as $p => $key ) {
			# do the actual searchs
			$p_parsed = preg_replace('/\./', '_', $p);
			$wgOut->addHTML( '<div class="spx_tab_content" id="sphinx-group-' . $p_parsed . '">' );
			$found = 0;
			$cl = $this->prepareSphinxClient( $term, $match_titles_only, $p );
			if ( $cl ) {
				$res = $cl->Query(
				...
				if ( $res === false ) {
					...
					$found = $this->wfSphinxDisplayResults( $term, $res, $cl, $p );
				}
			} else {
				wfDebug( wfMsg( 'sphinxClientFailed' ) . "\n" );
			}

			# prepare for the next search
			if ( $found ) {
				$this->createNextPageBar( $found, $term, $p );
			}
			$wgOut->addHTML( "</div>" );
		}
		$wgOut->addHTML( '</div>' );
	 ...
	 * @param int $limit
	 * @param int $offset
	 * @param int $group Which series to use from page array
	 * @access public
	 */
	function setLimitOffset( $limit, $offset = 0, $group ) {
		global $wgSphinxSearch_matches;

		$wgSphinxSearch_matches = intval( $limit );

		if ( $offset > 0 && $limit > 0 ) {
			$this->page[$group] = 1 + intval( $offset / $limit );
		}
	}
	 ...
	 * @return SphinxBFSearchResultSet
	 * @access public
	 */
	function searchText( $term, $titles_only = false ) {
		...
		if ( $res === false ) {
			...
			return new SphinxBFSearchResultSet( $term, $res, $cl );
		}
	}
	 ...
	 */
	function prepareSphinxClient( $term, $match_titles_only = false, $group ) {
		...
		# Always filter out redirects for Blender searches
		$cl->SetFilter( 'page_is_redirect', array( 0 ) );

		foreach ( $this->series as $sr ) {
			if ( $sr["series"] == $group ) {
				$cl->SetFilter( 'version', array($sr["crc32"]), false ); // Include
			} else {
				$cl->SetFilter( 'version', array($sr["crc32"]), true );  // Exclude
			}
		}

		$cl->SetSortMode( $wgSphinxSearch_sortmode, $wgSphinxSearch_sortby );
		$cl->SetLimits(
			( $this->page[$group] - 1 ) * $wgSphinxSearch_matches,
	...
	function wfSphinxDisplayResults( $term, $res, $cl, $group ) {
		...
		wfProfileIn(__METHOD__ . ":sphinxSearchWarning");
		if ($cl->GetLastWarning()) {
			$wgOut->addWikiText( wfMsg( 'sphinxSearchWarning', $cl->GetLastWarning() ) . "\n\n");
		}
		$found = $res['total_found'];
		wfProfileOut(__METHOD__ . ":sphinxSearchWarning");

		wfProfileIn(__METHOD__ . ":SuggestMode");
		if ( $wgSphinxSuggestMode ) {
			...
			if ( $didyoumean ) {
				$wgOut->addhtml( wfMsg( 'sphinxSearchDidYouMean' ) .
					...
					"page[$group]=1'>" . $didyoumean . '</a></b>'
				);
			}
		}
		wfProfileOut(__METHOD__ . ":SuggestMode");

		wfProfileIn(__METHOD__ . ":stats");
		$from = min(
			( ( $this->page[$group] - 1 ) * $wgSphinxSearch_matches ) + 1,
			$res['total']
		);
		$to = min(
			$this->page[$group] * $wgSphinxSearch_matches,
			$res['total']
		);
		...
		wfProfileOut(__METHOD__ . ":stats");

		$start_time = microtime( true );

		if ( isset( $res["matches"] ) && is_array( $res["matches"] ) ) {
			$wgOut->addWikiText( "----" );
			wfProfileIn(__METHOD__ . ":wfGetDB");
			$dbr = wfGetDB( DB_SLAVE );
			wfProfileOut(__METHOD__ . ":wfGetDB");
			$excerpts_opt = array(
			...
			foreach ( $res["matches"] as $doc => $docinfo ) {
				...
				wfProfileIn(__METHOD__ . ":selectField");
				...
				wfProfileOut(__METHOD__ . ":selectField");

				if ( $page_content ) {

					wfProfileIn(__METHOD__ . ":Title::newFromID");
					$title_obj = Title::newFromID( $doc );
					wfProfileOut(__METHOD__ . ":Title::newFromID");

					if ( is_object( $title_obj ) ) {
						wfProfileIn(__METHOD__ . ":getPrefixedText");
						$wiki_title = $title_obj->getPrefixedText();
						wfProfileOut(__METHOD__ . ":getPrefixedText");

						wfProfileIn(__METHOD__ . ":getPrefixedDBkey");
						$wiki_path = $title_obj->getPrefixedDBkey();
						$wgOut->addWikiText( "* <span style='font-size:110%;'>[[:$wiki_path|$wiki_title]]</span>" );
						wfProfileOut(__METHOD__ . ":getPrefixedDBkey");
						...
						wfProfileIn(__METHOD__ . ":Excerpts");
						...
						wfProfileOut(__METHOD__ . ":Excerpts");
					}
				}
			}
			$time = number_format( microtime( true ) - $start_time, 3);
			#$wgOut->addWikiText( wfMsg( 'sphinxSearchEpilogue', $time ) );
			wfProfileIn(__METHOD__ . ":wfDebug");
			wfDebug( "Database search time: $time" );
			wfProfileOut(__METHOD__ . ":wfDebug");
		}

		wfRunHooks( 'SphinxSearchAfterResults', array( $term, $this->page[$group] ) );
	...
	function getActionURL( $term, $group ) {
		...
		foreach ( $this->series as $s) {
			$qry .= "series{$s["series"]}={$s["checked"]}&amp;";
		}
		$qry .= "searchLang=" . $this->searchLang . "&amp;";
		
		# Add &page{group}= to $qry for every group EXCEPT the one passed as
		# $group, that will be dealt with in createNextPageBar() by string
		# concatenation means in it's own code
		foreach ( $this->page as $p => $n ) { // only use searched pages, not all
			if ( $p != $group ) {
				$qry .= "page{$p}={$n}";
			}
		}
	...
	function createNextPageBar( $found, $term, $group ) {
		global $wgOut, $wgSphinxSearch_matches;

		wfProfileIn(__METHOD__);

		$qry = $this->getActionURL( $term, $group );

		$display_pages = 10;
		$max_page = ceil( $found / $wgSphinxSearch_matches );
		$center_page = floor( ( $this->page[$group] + $display_pages ) / 2 );
	...
	<table border=\"0\" cellpadding=\"0\" width=\"1%\" cellspacing=\"0\">
		<tr align=\"center\" valign=\"top\">
			...
			if ( $first_page > 1 ) {
				$prev_page  = "<td>&#160;<a href='{$qry}&amp;page{$group}=";
				$prev_page .= ( $this->page[$group] - 1 ) . "'>" . wfMsg( 'sphinxPreviousPage' ) . "</a>&#160;</td>";
				$wgOut->addHTML( $prev_page );
			}
			for ( $i = $first_page; $i < $this->page[$group]; $i++ ) {
				$wgOut->addHTML( "<td>&#160;<a href='{$qry}&amp;page{$group}={$i}'>{$i}</a>&#160;</td>" );
			}
			$wgOut->addHTML( "<td>&#160;<b>{$this->page[$group]}</b>&#160;</td>" );
			for ( $i = $this->page[$group] + 1; $i <= $last_page; $i++ ) {
				$wgOut->addHTML( "<td>&#160;<a href='{$qry}&amp;page{$group}={$i}'>{$i}</a>&#160;</td>" );
			}
			if ( $last_page < $max_page ) {
				$next_page  = "<td>&#160;<a href='{$qry}&amp;page{$group}=";
				$next_page .= ( $this->page[$group] + 1 ) . "'>" . wfMsg( 'sphinxNextPage' ) . "</a>&#160;</td>";
	...
	function createNewSearchForm( $term ) {
		global $wgOut, $wgDisableInternalSearch, $wgSphinxSearch_mode, $wgSphinxMatchAll,
			...
			$wgSphinxSearchExtPath, $wgSphinxSearchJSPath, $wgRequest, $wgLang;
		...
		$searchField = strtolower( $search_title );
		$wgOut->addHTML( "<form id=\"spx_form\" action='$kiAction' method=\"GET\">
				...
				<input type='hidden' name='searchLang' value='" . $wgRequest->getVal("searchLang", $wgLang->getCode()) . "'>" );

		$wgOut->addHTML( "<div id=\"spx_match_extended\">" );
		...
		# get user settings for which series to search
		$wgOut->addHTML( "</div><div id=\"spx_series_search\">" );
		$wgOut->addHTML( "Series:<br />" );
		foreach ( $this->series as $sr ) {
			$checked = $sr["checked"] ? ' checked="checked"' : '';
			$wgOut->addHTML( "<label><input type='checkbox' value='1' name='series" . $sr["series"] . "'$checked />" . $sr["label"] . "</label><br />" );
		}

		# Add language dropdown
		$wgOut->addHTML( self::languageDropdown() );
		...
		$wgOut->addHTML( "<div id=\"poweredbysphinx\">" .
 ...
 * @ingroup Search
 */
class SphinxBFSearchResultSet extends SearchResultSet {
	var $mNdx = 0;
